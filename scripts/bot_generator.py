...
CONFESSION_STYLES = [
    ...
    "дивування — коли дивишся на неї і досі не можеш повірити своему щастю",
...
ПРАВИЛА ВИБОРУ ТВОРУ ТА ДАТИ:
1. Жанр — обирай різноманітно: вірш, роман, опера, балет, картина, симфонія, поема, відома пісня.
2. Автор має бути відомим, твір — реальним.
3. КРИТИЧНО: Рік має бути ОДНОЗНАЧНИМ та загальноприйнятим. Уникай творів, де рік написання та рік першої публікації чи першого релізу платівки відрізняються (якщо є суперечки чи різні трактування дати в джерелах — НЕ ОБИРАЙ цей твір). Потрібна лише одна залізобетонна дата (чотири цифри YYYY).

ПРАВИЛА ФОРМУВАННЯ ПИТАННЯ (ЧИСТИЙ ТЕКСТ):
1. Питання формулюй виключно українською мовою.
2. КАТЕГОРІЧНО ЗАБОРОНЕНО використовувати будь-які латинські літери (іноземні назви чи імена). Усі назви творів та імена авторів мають бути написані виключно українськими літерами: або у вигляді офіційного українського перекладу, або українською транслітерацією за звучанням (наприклад, замість "La Vie en rose" пиши Ла Ві ан роз).
3. КАТЕГОРІЧНО ЗАБОРОНЕНО використовувати будь-які лапки (внутрішні, зовнішні, одинарні, подвійні, ялинки "", „“, \\"\\"). Назву твору пиши просто з великої літери всередині речення без жодних лапок.
4. Дозволені розділові знаки: знак питання в кінці речення, коми всередині. Будь-які інші спецсимволи — табу.
"""
...
def polybius_encode(text):
    ...
    encoded = []
    # Захисне очищення: замінюємо дефіси на пробіли, а випадкові лапки повністю видаляємо
    cleaned_text = text.replace("-", " ").replace('"', '').replace("'", "").replace("`", "")
    
    for char in cleaned_text.upper():
        found = False
        for i, row in enumerate(grid):
            for j, val in enumerate(row):
                if char == val:
                    ...
                    break
            if found: break
        # Якщо в тексті все ж проскочить неузгоджений символ (цифра чи латиниця),
        # ми його просто ігноруємо, щоб не ламати структуру шифру штучними пробілами
    return " ".join(encoded)
